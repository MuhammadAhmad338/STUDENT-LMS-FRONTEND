...
} from "@/app/lib/Slices/enrollments/enrollmentSlice";
...
export default function CourseDetailPage() {
  ...
  /* ── not found ───────────────────────────────────────────────────── */
  if (courseStatus !== "loading" && !course && courses.length > 0) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-6 text-center">
        <div className="rounded-3xl border border-slate-200 bg-white p-10 shadow-sm shadow-slate-200 max-w-md">
          ...
          <h1 className="mt-4 text-2xl font-semibold text-slate-900">Course not found</h1>
          <p className="mt-2 text-slate-600">The course you're looking for doesn't exist or was removed.</p>
          <Link href="/courses" className="mt-6 inline-block rounded-xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-700">
            Browse all courses
          </Link>
        </div>
      </main>
            ...
            <div className="flex border-b border-slate-200 bg-white rounded-t-3xl border-t border-x border-slate-200/80">
              {[
                ...
                <button
                  ...
                  className={`px-6 py-4 text-sm font-semibold transition border-b-2 -mb-[2px] ${
                    activeTab === tab.id
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-2xl border transition-all ${
                            isOpen ? "border-cyan-300 bg-cyan-50/20" : "border-slate-200 bg-slate-50/50 hover:bg-slate-50"
                          }`}
                        >
                          <button
                            type="button"
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg text-xs font-bold ${
                                isOpen ? "bg-cyan-700 text-white" : "bg-slate-200 text-slate-700"
                              }`}>
                                {index + 1}
                              </span>
